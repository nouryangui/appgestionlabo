...
import { MemberPublicationFormComponent } from 'app/member-list/member-publication-form/member-publication-form.component';
...
export class PublicationListComponent implements OnInit {
 ...
 /* public doFilter = (value: string) => {
  ...
  onAddMemberToPub(idPublication:number)
  {
    const dialogRef = this.matDialog.open(MemberPublicationFormComponent, {
      disableClose: false,
      width: '30%',
      data: { publicationId: idPublication }
    });
    dialogRef.afterClosed().pipe(takeUntil(this._onDestroy)).subscribe(result => this.fetchDataSourcePaginator()
    );
  }
